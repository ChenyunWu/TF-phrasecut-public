...
def train(modelname, max_iter, snapshot, dataset, weights, setname, mu):
    tfmodel_folder = './_rmi/' + dataset + '/tfmodel/'
    ...
    data_loader = RMIRefVGLoader(split=setname)
    ...
    if weights == 'resnet':
        pretrained_model = './_rmi/external/TF-resnet/model/ResNet101_init.tfmodel'
        load_var = {var.op.name: var for var in tf.global_variables() if var.op.name.startswith('ResNet')}
    elif weights == 'deeplab':
        pretrained_model = './_rmi/external/TF-deeplab/model/ResNet101_train.tfmodel'
...
def test(modelname, iter, dataset, visualize, weights, setname, dcrf, mu):
    save_dir = None
    if visualize:
        save_dir = './_rmi/' + dataset + '/visualization/' + modelname + '_' + str(iter) + '/'
        if not os.path.isdir(save_dir):
            os.makedirs(save_dir)
    pretrained_model = './_rmi/' + dataset + '/tfmodel/' + dataset + '_' + weights + '_' + modelname + '_iter_' + str(
        iter) + '.tfmodel'
    ...
    while not data_loader.is_end:
        ...
        if visualize and np.random.random() < 0.02:
            visualize_seg(im, predicts, sent, save_dir)
            if dcrf:
                visualize_seg(im, predicts_dcrf, sent, save_dir)
...
def visualize_seg(im, predicts, sent, save_dir):
    im_seg = im / 2
    im_seg[:, :, 0] += predicts.astype('uint8') * 100
    plt.imshow(im_seg.astype('uint8'))
    plt.title(sent)
    plt.savefig(os.path.join(save_dir, '%s.jpg' % sent))
...
if __name__ == "__main__":
    ...
    # parser.add_argument('-d', type=str)  # 'Gref' 'unc' 'unc+' 'referit'  --> 'refvg'
    ...
    parser.add_argument('-t', type=str)  # 'train' 'val' 'test' 'miniv'
